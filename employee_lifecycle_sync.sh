...
# CMPE341 Linux Final Project
# 
# Purpose: Automate user account management based on HR CSV data
# Features: Onboarding, Offboarding, Change Detection, Reporting
################################################################################
...
log_command() {
    local timestamp=$(date '+%Y-%m-%d %H:%M:%S')
    echo -e "${BLUE}[$timestamp] [RUN] $1${NC}" | tee -a "$LOG_FILE"
}
...
normalize_csv() {
    ...
    # Read CSV, skip header, normalize (lowercase usernames, trim spaces)
    # Sort by employee_id for consistent comparison
    awk -F',' 'NR>1 {
        ...
        # Convert username and department to lowercase
        $2 = tolower($2);
        $4 = tolower($4);
        $5 = tolower($5);
        ...
        print $1","$2","$3","$4","$5
    }' "$input_file" | sort -t',' -k1 > "$output_file"
...
detect_changes() {
    ...
    # Normalize previous snapshot
    normalize_csv "$SNAPSHOT_FILE" "$TEMP_PREVIOUS"
    
    # Use comm to find differences
    # comm output: column 1 = only in file1, column 2 = only in file2, column 3 = in both
    
    # Added employees: in current, not in previous (column 2)
    comm -13 "$TEMP_PREVIOUS" "$TEMP_CURRENT" > "$TEMP_ADDED"
    
    # Removed employees: in previous, not in current (column 1)
    comm -23 "$TEMP_PREVIOUS" "$TEMP_CURRENT" > "$TEMP_REMOVED"
...
onboard_employee() {
    ...
    ((ADDED_COUNT++))
    log_success "Onboarding complete for $username"
}
...
main() {
    ...
    log "START lifecycle sync | file=$EMPLOYEES_CSV | dry_run=$dry_run"
